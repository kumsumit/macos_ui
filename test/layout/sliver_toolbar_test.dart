...
void main() {
  ...
  testWidgets('MediaQuery bottom padding is removed as expected', (
    tester,
  ) async {
    ...
    await tester.pumpWidget(
      MacosApp(
        home: MediaQuery(
          data: const MediaQueryData(padding: EdgeInsets.only(bottom: 30.0)),
          child: MacosScaffold(
            children: [
              ContentArea(
  ...
  testWidgets('Y-Offsets and sizes are correct', (tester) async {
    final ScrollController controller = ScrollController();
  ...
  testWidgets('Scrolling down while pinned=true keeps the toolbar in view', (
    tester,
  ) async {
    final ScrollController controller = ScrollController();
    await pumpScrollableWithSliverToolbar(tester, controller);
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.sendEventToBinding(
        testPointer.scroll(const Offset(0.0, -52.0)),
      );
